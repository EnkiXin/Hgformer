...
from recbole_gnn.model.general_recommender.nodeformer import NodeFormer
from graph_transformer.sgformer import SGFormer
from recbole_gnn.model.crossnodeformer import CrossNodeFormer
from recbole_gnn.model.crosssgformer import CrossSGFormer
...
class EucRecFormer(GeneralRecommender):
    input_type = InputType.PAIRWISE
    def __init__(self, config, dataset):
        super(EucRecFormer, self).__init__(config, dataset)
        self.config=config
        ...
        if config['transformer_type']=='nodeformer':
            self.transformer=NodeFormer(
                    config=self.config,
                    sformation=config['softmax_kernel_transformation'],
        ...
        elif config['transformer_type']=='sgformer':
            self.transformer=SGFormer(
        ...
        elif config['transformer_type'] == 'crossnodeformer':
            self.transformer=CrossNodeFormer(
                    config=self.config,
                    sformation=config['softmax_kernel_transformation'],
                    in_channels=config['latent_dim'],
                    out_channels=config['latent_dim'],
        ...
        elif config['transformer_type']=='crosssgformer':
            self.transformer=CrossSGFormer(
                                         in_channels=self.latent_dim,
                                         hidden_channels=self.latent_dim,
                                         num_layers=self.n_layers,
                                         num_heads=self.num_heads,
    ...
    def forward(self):
        ...
        if self.config['transformer_type']=='cross_nodeformer':
           ...
           return user_embeddings, item_embeddings,link_loss
            ...
            return user_all_embeddings, item_all_embeddings,link_loss
