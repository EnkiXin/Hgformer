...
from graph_transformer.hygformer_manifolds.hyp_layer import HypLinear, HypLayerNorm, HypActivation, HypDropout, HypNormalization, HypCLS
from graph_transformer.hygformer_manifolds.lorentz import Lorentz
from graph_transformer.gnns import GraphConv
...
class TransConv(nn.Module):
    def __init__(self,
                 ...
                 manifold_out,
                 in_channels,
                 hidden_channels, num_layers=2, num_heads=1,
                 dropout=0.5, use_bn=False, use_residual=True, use_weight=True, use_act=False, args=None):
        super().__init__()
    ...
    def forward(self, x_input):
        ...
        self.manifold_out._check_point_on_manifold(x)
...
class HypFormer(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.config=args
        self.manifold_in = Lorentz(k=float(self.config['curve']))
        # self.manifold_hidden = Lorentz(k=float(args.k_in))
        self.manifold_hidden = Lorentz(k=float(self.config['curve']))
        self.decoder_type = self.config['decoder_type']

        self.manifold_out = Lorentz(k=float(self.config['curve']))
        self.in_channels = in_channels
        self.hidden_channels = hidden_channels
        self.out_channels = out_channels
        ...
        self.trans_conv = TransConv(self.manifold_in,
                                    ...
                                    trans_use_act,
                                    self.config)
        self.graph_conv = GraphConv(in_channels,
                                    hidden_channels,
                                    gnn_num_layers,
                                    gnn_dropout,
                                    gnn_use_bn,
    ...
    def forward(self,all_embeddings, edge_index):
        x = self.trans_conv(all_embeddings)
