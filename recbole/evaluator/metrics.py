...
class TailHit(Hit):
    def calculate_metric(self, dataobject):
        tail_pos_index, _,_,_ = self.tail_analysis_used_info(dataobject)
        tail_result = self.metric_info(tail_pos_index)
        metric_dict = self.topk_result('tail_hit', tail_result)
        return metric_dict

class HeadHit(Hit):
    def calculate_metric(self, dataobject):
        _, _,head_pos_index,_ = self.tail_analysis_used_info(dataobject)
        result = self.metric_info(head_pos_index)
        metric_dict = self.topk_result('head_hit', result)
        return metric_dict

class Rank1Hit(Hit):
    def calculate_metric(self, dataobject):
        rank1index, _,_,_,_,_,_,_,_,_ = self.rank_tail_analysis_used_info(dataobject)
        result = self.metric_info(rank1index)
        metric_dict = self.topk_result('rank1hit', result)
        return metric_dict
class Rank2Hit(Hit):
    def calculate_metric(self, dataobject):
        _, _,rank2index,_,_,_,_,_,_,_ = self.rank_tail_analysis_used_info(dataobject)
        result = self.metric_info(rank2index)
        metric_dict = self.topk_result('rank2hit', result)
        return metric_dict
class Rank3Hit(Hit):
    def calculate_metric(self, dataobject):
        _, _,_,_,rank3index,_,_,_,_,_ = self.rank_tail_analysis_used_info(dataobject)
        result = self.metric_info(rank3index)
        metric_dict = self.topk_result('rank3hit', result)
        return metric_dict
class Rank4Hit(Hit):
    def calculate_metric(self, dataobject):
        _, _,_,_,_,_,rank4index,_,_,_ = self.rank_tail_analysis_used_info(dataobject)
        result = self.metric_info(rank4index)
        metric_dict = self.topk_result('rank4hit', result)
        return metric_dict
class Rank5Hit(Hit):
    def calculate_metric(self, dataobject):
        _, _,_,_,_,_,_,_,rank5index,_ = self.rank_tail_analysis_used_info(dataobject)
        result = self.metric_info(rank5index)
        metric_dict = self.topk_result('rank5hit', result)
        return metric_dict
...
class TailRecall(Recall):
    def calculate_metric(self, dataobject):
        tail_pos_index, tail_pos_len,_,_ = self.tail_analysis_used_info(dataobject)
        tail_result = self.metric_info(tail_pos_index, tail_pos_len)
        metric_dict = self.topk_result('tailrecall', tail_result)
        return metric_dict

class HeadRecall(Recall):
    def calculate_metric(self, dataobject):
        _,_,head_pos_index, head_pos_len = self.tail_analysis_used_info(dataobject)
        #print(head_pos_index)
        head_result = self.metric_info(head_pos_index, head_pos_len)
        metric_dict = self.topk_result('headrecall', head_result)
        return metric_dict


class Rank1Recall(Recall):
    def calculate_metric(self, dataobject):
        pos_index, pos_len,_,_,_,_,_,_,_,_ = self.rank_tail_analysis_used_info(dataobject)
        result = self.metric_info(pos_index, pos_len)
        metric_dict = self.topk_result('rank1recall', result)
        return metric_dict
class Rank2Recall(Recall):
    def calculate_metric(self, dataobject):
        _,_,pos_index, pos_len,_,_,_,_,_,_ = self.rank_tail_analysis_used_info(dataobject)
        result = self.metric_info(pos_index, pos_len)
        metric_dict = self.topk_result('rank2recall', result)
        return metric_dict
class Rank3Recall(Recall):
    def calculate_metric(self, dataobject):
        _,_,_,_,pos_index, pos_len,_,_,_,_ = self.rank_tail_analysis_used_info(dataobject)
        result = self.metric_info(pos_index, pos_len)
        metric_dict = self.topk_result('rank3recall', result)
        return metric_dict
class Rank4Recall(Recall):
    def calculate_metric(self, dataobject):
        _,_,_,_,_,_,pos_index, pos_len,_,_ = self.rank_tail_analysis_used_info(dataobject)
        result = self.metric_info(pos_index, pos_len)
        metric_dict = self.topk_result('rank4recall', result)
        return metric_dict

class Rank5Recall(Recall):
    def calculate_metric(self, dataobject):
        _,_,_,_,_,_,_,_,pos_index, pos_len = self.rank_tail_analysis_used_info(dataobject)
        result = self.metric_info(pos_index, pos_len)
        metric_dict = self.topk_result('rank5recall', result)
        return metric_dict
...
class RankPercentage(AbstractMetric):
    metric_type = EvaluatorType.RANKING
    metric_need = ['rec.items', 'data.count_items']
    def __init__(self, config):
        super().__init__(config)
    ...
    def rank_tail_percentage_used_info(self, dataobject):
        """Get the bool matrix indicating whether the corresponding item is positive
        and number of positive items for each user.
        """
        rank1item = dataobject.get('rec.rank1item')
        rank2item = dataobject.get('rec.rank2item')
        rank3item = dataobject.get('rec.rank3item')
        rank4item = dataobject.get('rec.rank4item')
        rank5item = dataobject.get('rec.rank5item')
        rank5result = dataobject.get('rec.rank5result')
        return rank1item, rank2item,rank3item,rank4item,rank5item,rank5result


    def used_info(self, dataobject):
        """Get the matrix of recommendation items and number of items in total item set."""
        item_matrix = dataobject.get('rec.items')
#        acc=dataobject.get('rec.rank5acc')
        rank1item, rank2item,rank3item,rank4item,rank5item,rank5result=self.rank_tail_percentage_used_info(dataobject)
        return rank1item, rank2item,rank3item,rank4item,rank5item,item_matrix,rank5result

    def calculate_metric(self,dataobject):
        result=[]

        # rank5result:
        rank1item, rank2item,rank3item,rank4item,rank5item,item_matrix,rank5result = self.used_info(dataobject)
        recommended_items = item_matrix.flatten()
        result.append(torch.isin(recommended_items, rank1item).sum().item()/ len(recommended_items))
        result.append(torch.isin(recommended_items, rank2item).sum().item() / len(recommended_items))
        result.append(torch.isin(recommended_items, rank3item).sum().item() / len(recommended_items))
        result.append(torch.isin(recommended_items, rank4item).sum().item() / len(recommended_items))
        result.append(torch.isin(recommended_items, rank5item).sum().item() / len(recommended_items))

        trank5_topk_idx, rank5_pos_len_list = torch.split(rank5result, [max(self.topk), 1], dim=1)

        result.append(torch.sum(trank5_topk_idx)/torch.sum(rank5_pos_len_list))
        result.append(torch.sum(trank5_topk_idx) /torch.numel(trank5_topk_idx) )
        #print(torch.sum(trank5_topk_idx))
        #print(torch.sum(rank5_pos_len_list))
        metric_dict = {'rankpercentage': result}
