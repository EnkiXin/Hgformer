...
class GeneralRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.user_interations,self.item_interations=dataset.get_interactions()
        if config['tail_analysis']==True:
            self.head_item, self.tail_item=self.get_head_and_tail(self.item_interations,config['head_ratio'])
        if config['popularity_analysis']==True:
            self.rank1item, self.rank2item,self.rank3item,self.rank4item,self.rank5item = self.get_popularity_rank_items(self.item_interations)
        # load parameters info
        self.device = config['device']
        self.popularity = self.get_popularity(self.item_interations)
    def get_head_and_tail(self, item_interations, ratio):
        unique, counts = torch.unique(item_interations, return_counts=True)
        frequency_dict = dict(zip(unique.tolist(), counts.tolist()))
        sorted_items = sorted(frequency_dict.items(), key=lambda item: item[1], reverse=True)
        threshold_index1 = int(0.2 * len(sorted_items))
        threshold_index2 = int(0.4 * len(sorted_items))
        threshold_index3 = int(0.6 * len(sorted_items))
        threshold_index4 = int(0.8 * len(sorted_items))

        rank1item = self.sort_unique(torch.tensor([item[0] for item in sorted_items[:threshold_index1]]))
        rank2item = self.sort_unique(torch.tensor([item[0] for item in sorted_items[threshold_index1:threshold_index2]]))
        rank3item = self.sort_unique(torch.tensor([item[0] for item in sorted_items[threshold_index2:threshold_index3]]))
        rank4item = self.sort_unique(torch.tensor([item[0] for item in sorted_items[threshold_index3:threshold_index4]]))
        rank5item = self.sort_unique(torch.tensor([item[0] for item in sorted_items[threshold_index4:]]))
        tail_item = torch.cat((rank2item, rank3item,rank4item,rank5item), dim=0)
        return rank1item, tail_item

    def get_popularity_rank_items(self, item_interations):
        unique, counts = torch.unique(item_interations, return_counts=True)
        frequency_dict = dict(zip(unique.tolist(), counts.tolist()))
        sorted_items = sorted(frequency_dict.items(), key=lambda item: item[1], reverse=True)
        threshold_index1 = int(0.2 * len(sorted_items))
        ...
        rank4item = self.sort_unique(torch.tensor([item[0] for item in sorted_items[threshold_index3:threshold_index4]]))
        rank5item = self.sort_unique(torch.tensor([item[0] for item in sorted_items[threshold_index4:]]))
        return rank1item, rank2item,rank3item,rank4item,rank5item

    def get_popularity(self,item_interations):
        unique, counts = torch.unique(item_interations, return_counts=True)
        return counts


    def sort_unique(self, tensor):
        tensor_unique = torch.tensor(list(set(tensor.tolist())))
        sorted_tensor = torch.sort(tensor_unique).values
