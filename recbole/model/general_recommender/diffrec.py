...
class DNN(nn.Module):
    ...
    def __init__(
        ...
        self.norm = norm
        self.emb_layer = nn.Linear(self.time_emb_dim,
                                   self.time_emb_dim)
    ...
    def forward(self, x, timesteps):
        # 输入是加噪后的embeddings和step数
        # 对timestep进行embedding
...
class DiffRec(GeneralRecommender,
    ...
    def calculate_for_diffusion(self):
        ...
        # alphas_cumprod=[0.9,0.9*0.8,0.9*0.8*0.7,0.9*0.8*0.7*0.6,0.9*0.8*0.7*0.6*0.5]
    ...
    def p_sample(self, x_start):
        ...
        if self.noise_scale == 0.0:
            ...
            return x_t
        for i in indices:
            # t:给x_t的每个元素都增加一个step t的噪声
            t = torch.tensor([i] * x_t.shape[0]).to(x_start.device)
            # x_t：noisy的interactions，t：steps
    ...
    def calculate_loss(self, interaction):
        # user:是一个batch的user的interactions
        user = interaction[self.USER_ID]
        # x_start：获取这个batch的users的全部interactions（batch_size*num_items）
        x_start = self.get_rating_matrix(user)
        batch_size, device = x_start.size(0), x_start.device
        # ts:一个batch大小，0~steps之间的一个随机整数 pt：一个batch大小全1的整数
        ts, pt = self.sample_timesteps(batch_size, device, "importance")
        noise = torch.randn_like(x_start)
        if self.noise_scale != 0.0:
            # x_t：加完noise的interaction matrix，实际上是noise和x_start的一个加权平均
            x_t = self.q_sample(x_start, ts, noise)
        else:
            x_t = x_start
        # mlp：输入对应的step数和对应的第几个step的noise，用mlp预测一个结果
        ...
        reloss = self.reweight_loss(x_start,
                                    x_t,
                                    mse,
                                    ts,
                                    target,
    ...
    def q_sample(self,
                 ...
                 t,
                 noise=None):
        # t：time_step，看是加的第几个step的noise
        if noise is None:
            noise = torch.randn_like(x_start)
        assert noise.shape == x_start.shape
        # value:原本的user的interaction的值*每个step的 根号alpha
        # sqrt_alphas_cumprod:alpha的累乘
        # t:time_step
        # x_start:未加噪状态
        # value:对应时间点的value的权重
        value=self._extract_into_tensor(self.sqrt_alphas_cumprod,t,x_start.shape)* x_start
        # 每个step的noise*每个step的 根号（1-alpha）
        weighted_noise=self._extract_into_tensor(self.sqrt_one_minus_alphas_cumprod,t,x_start.shape)* noise
        noised_value=value+weighted_noise
        return (noised_value)
    ...
    def p_mean_variance(self, x, t):
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        # 输入noisy的interactions和step t，得到还原后的data
        ...
        if self.mean_type == ModelMeanType.START_X:
            # model预测出的复原后的interaction matrix的形状
