...
class DMF(GeneralRecommender):
    ...
    def full_sort_predict(self, interaction):
        ...
        if self.config['tail_analysis'] == True:
            return self.head_item, self.tail_item, similarity.view(-1)
        elif self.config['popularity_analysis'] == True:
            return self.rank1item, self.rank2item, self.rank3item, self.rank4item, self.rank5item, similarity.view(-1)
        else:
