...
from hyperbolic_gnn.model.rgd.rsgd import RiemannianSGD
"""
Trainer的抽象类，主要实现两个功能，
（1）fit函数，送入一个全部的训练数据，进行一个epoch的训练
（2）evaluate函数，验证效果如何
这里先写一个Trainer的抽象类，然后写一个trainer，复写这里需要实现的函数
"""
class AbstractTrainer(object):
    r"""
    Trainer Class is used to manage the training and evaluation processes of recommender system models.
...
class Trainer(AbstractTrainer):
    ...
    """
    以下，为了实现fit和evaluate的功能，在Trainer类中定义一些辅助主要功能实现的函数
    """
    def _build_optimizer(self, **kwargs):
        ...
        if learner.lower() == 'adam':
            ...
            optimizer =RiemannianSGD (self.model.parameters(), lr=learning_rate, weight_decay=weight_decay,momentum=self.momentum)
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        """
        即对dataloader进行循环，遍历里面的每一个batch
        """
        for batch_idx, interaction in enumerate(iter_data):
            # target_user_id,target_book_name,target_rating,target_label,source_user_id,source_ISBN,source_rating,source_label
    ...
    """
    实际用于训练的函数，输入是在data_preparation中住准备好的train_data，valid_data的datasampler
    valide_data是用于确定是否需要early_stopping的
    """
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        valid_step = 0
        """
        以下循环跑epoch轮，准确说是从第start_epoch轮跑到epochs轮
        """
        for epoch_idx in range(self.start_epoch, self.epochs):
            # train
            training_start_time = time()
            """
            _train_epoch函数做的事情是将train_data完整地跑一个epoch，然后得到一个loss，核心步骤就是下面的这一个函数
            所以整个fit函数的核心就是_train_epoch
            """
    ...
    def _full_sort_batch_eval(self, batched_data):
        # 输入：一个batch的user_id
        # 输出：
        #     interaction：一个batch的user_id
        #     scores,[n_batch_users,n_all_items]大小的scores矩阵
        #     positive_u,该batch的users的id
        #     positive_i：该batch的users对应的正样本
        interaction, history_index, positive_u, positive_i = batched_data
        try:
            # 送入一个batch的user_id，让这个batch的user与全部items做比对，计算出一个得分，生成一个得分矩阵
            # [n_batch_users,n_all_items]大小的矩阵
            if self.config['tail_analysis']==True:
                head_item,tail_item,scores = self.model.full_sort_predict(interaction.to(self.device))
        ...
        # 因为在整个数据中，0号user和0号item都是不存在的，因此不参与推荐
        scores[:, 0] = -np.inf
        if history_index is not None:
            scores[history_index] = -np.inf
        if self.config['tail_analysis'] == True:
           return head_item,tail_item,interaction, scores, positive_u, positive_i
        elif self.config['popularity_analysis'] == True:
            return rank1item, rank2item, rank3item, rank4item, rank5item, interaction, scores, positive_u, positive_i
        else:
            return interaction, scores, positive_u, positive_i

    def _neg_sample_batch_eval(self, batched_data):
        # interaction：是给model输入的数据
        # 包括target_user_id,target_book_name,target_rating,None
        interaction, row_idx, positive_u, positive_i = batched_data
        batch_size = interaction.length
        if batch_size <= self.test_batch_size:
            # origin_scores是通过模型预测出来的结果
    ...
    def evaluate(self, eval_data, load_best_model=True, model_file=None, show_progress=False):
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            ...
            # positive_i：该batch的users对应的正样本
            if self.config['tail_analysis'] == True:
                head_item,tail_item,interaction, scores, positive_u, positive_i = eval_func(batched_data)
            elif self.config['popularity_analysis']==True:
                rank1item, rank2item, rank3item,rank4item, rank5item, interaction, scores, positive_u, positive_i = eval_func(batched_data)
            else:
            ...
            if self.config['tail_analysis']==True:
               ...
               self.eval_collector.longtail_analysis(tail_item,
                                                     head_item,
                                                     scores,
                                                     interaction,
                                                     positive_u,
                                                     positive_i)
            if self.config['popularity_analysis']==True:
               self.eval_collector.rank_tail_analysis(rank1item,
                                                      rank2item,
                                                      rank3item,
                                                      rank4item,
                                                      rank5item,
...
class KGATTrainer(Trainer):
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        kg_total_loss = super()._train_epoch(train_data, epoch_idx, loss_func=self.model.calculate_kg_loss, show_progress=show_progress)
