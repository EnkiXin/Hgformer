...
class Interaction(object):
    """
    将DataFrame中的列保存为字典的形式，然后将该字典划分为minibatch，每个minibatch就是一个输入进model的interaction
    """
    """The basic class representing a batch of interaction records.
    在recbole的数据结构中，interaction就是一个minibatch
    Note:
