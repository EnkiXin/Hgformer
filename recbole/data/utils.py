...
def data_aug(config, inter_feat):
    aug_path = config['data_path'] + '/data_aug.npy'
    import os
    if os.path.exists(aug_path):
        ...
        for index, item_id in enumerate(target_item):
            ...
            data_aug_index.append(all_index_same_id_wo_self)
...
def data_preparation(config, dataset):
    ...
    if dataloaders is not None:
        ...
        if config['aug'] == 'cts':
            data_aug_index = data_aug(config, train_dataset.inter_feat)
            setattr(train_dataset, 'data_aug_index', data_aug_index)
